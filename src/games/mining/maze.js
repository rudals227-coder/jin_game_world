...
// 팩맨 스타일 레벨 (19 x 21). 두꺼운 블록 + 좁은 통로 + 중앙 몬스터 구역 + 좌우 워프.
// 클리어 = 모든 광석(.) 수집. 보물(랜덤 효과)은 맵 타일이 아니라 index.js에서 랜덤 위치 생성.
// 연결성 보장: 가로 통로 행(1,4,7,10,13,16,19)과 좌우 세로 통로(col1,col17)가 항상 열려 있음.
// P=플레이어, 1=추적형, 2=배회형, D=다이너마이트(파워), T=터널, %=파기 가능 벽(세로 지름길).
export const LEVEL = [
  '###################',
  '#D...............D#',
  '#..##.%.###.%.##..#',
...
// 남은 광석 수 (클리어 조건: 0이 되면 승리)
export function oreRemaining(grid) {
  let n = 0;
  for (const row of grid) for (const t of row) if (t === TILE.ORE) n++;
  return n;
}
